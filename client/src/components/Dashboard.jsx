...
export default function Dashboard() {
  ...
  useEffect(() => {
    getResumes()
      .then((res) => setResumeCount(res.data.length))
      .catch(console.error);
    getApplications()
      .then((res) => {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        <div className="border border-[#222222] p-8 glass-panel-hover">
          <h3 className="text-[#888888] text-[10px] tracking-widest uppercase mb-4">
            Neural Maps
          </h3>
        ...
        {recentApps.length > 0 ? (
          <div className="flex flex-col">
            ...
            {recentApps.map((app) => (
              ...
              >
                <div className="col-span-4 text-white text-sm font-serif truncate pr-4">
                  {app.jobId?.title || "Target Role"}
                </div>
                <div className="col-span-4 text-[#888888] text-[10px] uppercase tracking-widest truncate pr-4">
                  {app.jobId?.company || "Target Company"}
                </div>
                <div
                  className={`col-span-2 text-[10px] uppercase tracking-widest ${app.matchScore ? "text-[#D4AF37]" : "text-[#555555]"}`}
                >
