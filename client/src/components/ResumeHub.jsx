...
export default function ResumeHub() {
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      setJobs(prev => [...prev, { id: res.data.queueJobId, type: 'parse-resume', resumeId: res.data.resumeId }]);
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="mb-16">
        <h2 className="text-5xl font-serif text-white mb-4 tracking-tighter">Neural Hub</h2>
        <p className="text-[10px] tracking-widest uppercase text-[#888888]">Ingest and Process Document Vectors</p>
      </div>
      ...
      {parsedResumes.length > 0 && (
        <div className="space-y-8">
          <h3 className="text-white text-[10px] tracking-widest uppercase mb-6 border-b border-[#222222] pb-4">Indexed Profiles</h3>
          {parsedResumes.map((resume, idx) => (
            <div key={idx} className="border border-[#222222] p-8 glass-panel-hover">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <p className="text-[10px] tracking-widest uppercase text-[#555555] mb-2">IDENTIFIER</p>
                  <p className="text-white font-serif text-2xl">{resume?.candidate_name || "Unknown Entity"}</p>
                </div>
                <div>
                  <p className="text-[10px] tracking-widest uppercase text-[#555555] mb-2">DETECTED SKILLS</p>
                  <div className="flex flex-wrap gap-2">
                    {[...(resume?.metrics?.technical_skills || []), ...(resume?.metrics?.soft_skills || [])].map(skill => (
                      <span key={skill} className="border border-[#333333] text-[#888888] text-[10px] uppercase px-3 py-1">{skill}</span>
                    ))}
                  </div>
                </div>
              </div>
