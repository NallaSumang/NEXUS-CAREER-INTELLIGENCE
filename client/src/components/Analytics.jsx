...
export default function Analytics() {
  ...
  return (
    ...
    >
      <div className="mb-16">
        <h2 className="text-5xl font-serif text-white mb-4 tracking-tighter">Trajectory</h2>
        <p className="text-[10px] tracking-widest uppercase text-[#888888]">Career Roadmap & Statistical Insight</p>
      </div>
      ...
      <div className="mb-16">
        ...
        >
          {analyticsData ? 'Refresh Trajectory Analysis' : 'Generate Trajectory Analysis'}
        </button>
      </div>
      ...
      {analyticsData ? (
        <div className="border border-[#222222] p-8 glass-panel-hover">
          <div className="flex justify-between items-center border-b border-[#222222] pb-4 mb-6">
            <h3 className="text-white text-[10px] tracking-widest uppercase">Roadmap Insights</h3>
            <button 
          ...
          {analyticsData.summary && (
            <div className="mb-8">
              <p className="text-[10px] tracking-widest uppercase text-[#555555] mb-2">EXECUTIVE SUMMARY</p>
              <p className="text-[#888888] font-serif text-lg leading-relaxed">{analyticsData.summary}</p>
            </div>
          )}

          {analyticsData.strengths && analyticsData.strengths.length > 0 && (
            <div className="mb-8">
              <p className="text-[10px] tracking-widest uppercase text-[#555555] mb-4">IDENTIFIED STRENGTHS</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {analyticsData.strengths.map((str, idx) => (
                  <div key={idx} className="border border-[#333333] p-4 text-[#888888] text-sm">
                    {str}
                  </div>
                ))}
              </div>
          ...
          {analyticsData.weaknesses && analyticsData.weaknesses.length > 0 && (
            <div className="mb-8">
              <p className="text-[10px] tracking-widest uppercase text-red-500 mb-4">VULNERABILITIES</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {analyticsData.weaknesses.map((w, idx) => (
                  <div key={idx} className="border border-red-500 p-4 text-[#888888] text-sm">
                    {w}
                  </div>
                ))}
              </div>
          ...
          {analyticsData.recommended_actions && (
            <div className="mt-8 pt-8 border-t border-[#222222]">
              <p className="text-[10px] tracking-widest uppercase text-[#D4AF37] mb-4">SUGGESTED VECTORS</p>
        ...
        <div className="border border-[#222222] p-16 text-center glass-panel-hover flex flex-col items-center">
          <span className="text-[#555555] text-[10px] tracking-widest uppercase mb-4 block">MODULE INACTIVE</span>
          <h3 className="text-white font-serif text-2xl mb-4">Awaiting Telemetry Data</h3>
          <p className="text-[#888888] max-w-md mx-auto">
            Trigger the analysis above. The Career Roadmap agent requires processed applications and matches to generate high-confidence trajectory insights.
          </p>
        </div>
      )}
    </motion.div>
