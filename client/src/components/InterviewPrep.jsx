...
import { generateInterviewPrep, getApplications, getInterviewNotes } from '../api';
...
export default function InterviewPrep() {
  ...
  useEffect(() => {
    if (applicationId) {
      ...
      getInterviewNotes(applicationId).then(res => {
  ...
  const handleGeneratePrep = async () => {
    ...
    try {
      ...
      setJobs(prev => [...prev, { id: res.data.queueJobId, type: 'interview-prep' }]);
    } catch (err) {
  ...
  const handleDownloadQuestions = () => {
    ...
    const content = interviewData.questions.map((q, i) => `Q${i+1} [${q.category || q.type || 'BEHAVIORAL'}]: ${q.question}\nRationale: ${q.why_asked || q.rationale}\n\n`).join('');
  ...
  return (
    ...
    >
      <div className="mb-16">
        <h2 className="text-5xl font-serif text-white mb-4 tracking-tighter">Combat Prep</h2>
        <p className="text-[10px] tracking-widest uppercase text-[#888888]">Simulated Interview Coach</p>
      </div>

      <div className="border border-[#222222] p-8 mb-16">
        <h3 className="text-white text-[10px] tracking-widest uppercase mb-6">Target Selection</h3>
        
        {applications.length === 0 ? (
          <div className="text-red-500 text-[10px] uppercase tracking-widest mb-6">No targets tracked. Add one in Active Targets.</div>
        ) : (
          <select 
          ...
          >
            {applications.map(app => (
              <option key={app._id} value={app._id}>
                {app.jobId?.title || "Target Role"} at {app.jobId?.company || "Company"} ({new Date(app.createdAt).toLocaleDateString()})
              </option>
            ))}
          </select>
        )}
      ...
      {interviewData && (
        <div className="mb-16 space-y-8">
          <div className="flex justify-between items-center border-b border-[#222222] pb-4 mb-6">
            <h3 className="text-white text-[10px] tracking-widest uppercase">Simulated Questions</h3>
            <button 
          ...
          {(interviewData.questions || []).map((q, idx) => (
            <div key={idx} className="border border-[#222222] p-8 glass-panel-hover">
              <p className="text-[10px] tracking-widest uppercase text-[#555555] mb-2">{q.category || q.type || 'BEHAVIORAL'} QUESTION</p>
              ...
              <div className="bg-[#111111] p-4 text-[#888888] text-sm font-mono border-l-2 border-[#555555]">
                <span className="text-[10px] tracking-widest uppercase text-[#555555] block mb-2">EXPECTED VECTOR (RATIONALE)</span>
