...
async def compute_match(req: MatchRequest):
    with open("prompts/match_score.txt", "r") as f:
        template = f.read()
    
    prompt = template.replace("{resume_json}", json.dumps(req.resumeJson))\
                     .replace("{job_requirements}", json.dumps(req.jobRequirements))
