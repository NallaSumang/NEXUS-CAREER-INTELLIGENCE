...
router.post('/match', verifyToken, async (req, res) => {
  try {
    ...
    if (!jobId && !jobDescription) return res.status(400).json({ error: 'Missing jobId or jobDescription' });

    const resume = await Resume.findById(resumeId);
    if (!resume || resume.userId.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Forbidden' });
    ...
    let application = await Application.findOne({ userId: req.user._id, jobId: finalJobId, resumeId });
    if (!application) {
      application = new Application({ userId: req.user._id, jobId: finalJobId, resumeId, status: 'Saved' });
      await application.save();
    }

    let queueJobId = `mock_job_${Date.now()}`;
    try {
      const jobReq = await aiQueue.add('compute-match', { resumeId, jobId: finalJobId, userId: req.user._id, applicationId: application._id });
      queueJobId = jobReq.id;
    } catch (e) {
      console.log('Redis down. Mocking compute-match.');
    }

    res.json({ applicationId: application._id, jobStatus: 'queued', queueJobId });
  } catch (error) {
...
router.post('/cover-letter', verifyToken, async (req, res) => {
  try {
    ...
    if (!application || application.userId.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Forbidden' });

    let queueJobId = `mock_job_${Date.now()}`;
    try {
      const jobReq = await aiQueue.add('gen-cover-letter', { applicationId, userId: req.user._id });
      queueJobId = jobReq.id;
    } catch (e) {
...
router.post('/interview-prep', verifyToken, async (req, res) => {
  try {
    ...
    let queueJobId = `mock_job_${Date.now()}`;
    try {
      const jobReq = await aiQueue.add('interview-prep', { applicationId, userId: req.user._id });
      queueJobId = jobReq.id;
    } catch (e) {
...
router.get('/status/:jobId', verifyToken, async (req, res) => {
  try {
    if (req.params.jobId.startsWith('mock_job_')) {
      ...
      if (elapsed < 4000) {
        return res.json({ state: 'active', progress: 50, result: null, failedReason: null });
      }
      return res.json({ state: 'completed', progress: 100, result: { mocked: true }, failedReason: null });
    }
...
router.post('/analytics', verifyToken, async (req, res) => {
  try {
    ...
    if (applications.length === 0) return res.status(400).json({ error: 'Not enough data to analyze.' });
...
router.get('/analytics', verifyToken, async (req, res) => {
  try {
    ...
    const analytics = await AIHistory.findOne({ userId: req.user._id, agentType: 'analytics' }).sort({ createdAt: -1 });
