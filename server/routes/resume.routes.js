  ...
  async (req, res) => {
    try {
      ...
      const uploadStream = bucket.openUploadStream(
        `resume_${Date.now()}_${req.file.originalname}`,
      );
      ...
      uploadStream.on("finish", async () => {
        try {
          ...
          try {
            ...
            const jobReq = await resumeQueue.add(
              "parse-resume",
            ...
            console.log(
              "[Fallback] Redis unavailable. Processing resume directly...",
            );
            ...
            (async () => {
              try {
                ...
                console.log(
                  "[Fallback] Resume processed successfully without Redis.",
                );
              } catch (fallbackErr) {
                console.error(
                  "[Fallback] Direct processing failed:",
                  fallbackErr.message,
                );
                await Resume.findByIdAndUpdate(resume._id, {
                  parseStatus: "failed",
          ...
          res
            .status(500)
            .json({ error: `Database save failed: ${err.message}` });
        }
      });
      ...
      uploadStream.on("error", (err) => {
        ...
        res
          .status(500)
          .json({ error: `Failed to write file to database: ${err.message}` });
      });
    } catch (error) {
