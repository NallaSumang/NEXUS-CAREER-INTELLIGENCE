...
const matchWorker = new Worker('ai-jobs', async (job) => {
  ...
  try {
    if (job.name === 'compute-match') {
      ...
      if (!currentJobDoc.parsedRequirements || Object.keys(currentJobDoc.parsedRequirements).length === 0) {
        console.log(`[Job Parser] Parsing raw job description for Job ${jobId}`);
        const parseRes = await axios.post(`${aiServiceUrl}/agents/parse-job`, {
          description: currentJobDoc.description
        }, { timeout: 60000 });
        
        currentJobDoc = await Job.findByIdAndUpdate(jobId, {
        ...
        await AIHistory.create({
          ...
          promptSnapshot: JSON.stringify({ description: currentJobDoc.description }),
      ...
      res = await axios.post(`${aiServiceUrl}/agents/compute-match`, {
      ...
      res = await axios.post(`${aiServiceUrl}/agents/generate-cover-letter`, {
        resumeJson: resume.parsedJson || {},
      ...
      res = await axios.post(`${aiServiceUrl}/agents/generate-interview-prep`, {
        resumeJson: resume.parsedJson || {},
        jobTitle: jobDoc.title,
        company: jobDoc.company,
      ...
      agentType = 'analytics';
      res = await axios.post(`${aiServiceUrl}/agents/generate-insights`, {
